...
flipRandom = let flipNum :: (Num a) => Random (a -> a)
                 flipNum = (\pos x -> if pos then x else (-x)) <$> (Random uniform)
...
sparseRandomProjection :: forall n m r a. (KnownNat n, KnownNat m, KnownNat r, n <= m, Num a) =>
                          Proxy n -> Random (Vector m a -> Vector r a)
sparseRandomProjection p = oneToMany <$>
